...
public class Brain {
	...
	//Remembering neutral bots
	private HashSet<MapLocation> neutralBots = new HashSet<MapLocation>();
	
	public MapLocation[] getNeutBots(){
		return neutralBots.toArray(new MapLocation[neutralBots.size()]);
	}
	
	public void addNeutBot(MapLocation loc){
		neutralBots.add(loc);
	}
	
	public void removeNeutBot(MapLocation loc){
		neutralBots.remove(loc);
	}
	
	public Boolean isNeutNew(MapLocation loc){
		return !(neutralBots.contains(loc));
	}
