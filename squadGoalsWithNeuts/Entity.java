...
import squadGoalsWithNeuts.Brain;
/*
...
//					Entity.signalMessageLocation(rc, zombie.location, (int) (1.3*rc.getLocation().distanceSquaredTo(brain.getStartingLocation())));
	...
	public static void searchForNeutBots(RobotController rc, Brain brain) throws GameActionException {
		RobotInfo[] neutsWithinRange = rc.senseNearbyRobots(rc.getType().sensorRadiusSquared, Team.NEUTRAL);
		for (RobotInfo neut : neutsWithinRange) {
			if (neut.type == RobotType.TURRET || neut.type == RobotType.TTM) {
				if (brain.isDenNew(neut.location)){
					brain.addNeutBot(neut.location);
//					Entity.signalMessageLocation(rc, zombie.location, (int) (1.3*rc.getLocation().distanceSquaredTo(brain.getStartingLocation())));
					Squad.shareNeutLocation(rc, brain, neut.location, 
							(int) (1.3*rc.getLocation().distanceSquaredTo(brain.getStartingLocation())));
					rc.setIndicatorString(1, "Found neutral bot at " + neut.location.x + ", " + neut.location.y);
				}
			}
		}
	}
	...
	public static void updateNeutLocations(RobotController rc, Brain brain) throws GameActionException {
		for (MapLocation neut : brain.getNeutBots()){
			if (rc.canSenseLocation(neut)){
				RobotInfo maybeNeut = rc.senseRobotAtLocation(neut);
				if (maybeNeut == null || maybeNeut.team==Team.NEUTRAL){
					brain.removeDenLocation(neut);
					if (brain.goalLocation.equals(neut)){
						brain.goalLocation = null;
					}
				}
			}
		}
		searchForNeutBots(rc, brain);
